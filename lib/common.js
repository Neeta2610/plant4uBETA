...
const updateTotalCart = async (req, res) => {
    ...
    var offersproduct = ["5fb80e81f929c311e883a9f8","5fb80d3cf929c311e883a9f7","5fb80bd9f929c311e883a9f5","5fb7de0ef929c311e883a9cc","5fb77644c75ee058f4633a3d","5fb7754dc75ee058f4633a3a","5fb773f1c75ee058f4633a35","5fb77282c75ee058f4633a2e","5fb77214c75ee058f4633a2c","5fbcab6e00292d5788c35988","5fbcab5400292d5788c35987"];
    var includesproduct = false;
    var freepotincludes = false;
    var fixedofferamount = 1000;
    var freepotid = "5fbcac4200292d5788c35989";
    Object.keys(req.session.cart).forEach((item) => {
        if(!req.session.cart[item].free)
        {
        ...
        if(offersproduct.includes(String(req.session.cart[item].productId))) {
            includesproduct = true;
        }
        if(freepotid == String(req.session.cart[item].productId)) {
            freepotincludes = true;
        }
    });
    if(!includesproduct && freepotincludes){
        delete req.session.cart["freepot"]
        freepotincludes = false;
    }

    // Update the total items in cart for the badge
    if(freepotincludes){
        req.session.totalCartItems = Object.keys(req.session.cart).length - 1;
    }
    else{
    ...
    if(includesproduct && !freepotincludes) {
        var freeproduct = await db.products.findOne({_id: getId("5fbcac4200292d5788c35989")});
        const productObj = {};
        productObj.productId = "5fbcac4200292d5788c35989";
        productObj.title = "White Buddha Statue";
        productObj.quantity = 1;
        productObj.totalItemPrice = 500;
        productObj.productDescription = "Ceramic Pot allow air and water movement through the sides of the pot. Air movement stimulates root growth, which results in healthier plants. These ceramic planters offer a beautifully organic shape that looks stunning indoors or outdoor. These ceramic planters with plants like small succulents and cactus will give a sparkling look to your home decor.";
        productObj.productImage = freeproduct.productImage;
        productObj.free = true;
        if(freeproduct.productPermalink){
            productObj.link = freeproduct.productPermalink;
        }else{
            productObj.link = freeproduct._id;
        }
        

        // merge into the current cart
        req.session.cart["freepot"] = productObj;
    }
};
 ...
 * @param  {object} sort // The mongo sort
 */
const paginateProducts = (frontend, db, page, query, sort,paginate = 8) => {
    const config = getConfig();
    let numberItems = 10;
    if(frontend){
        numberItems = paginate;
    }
